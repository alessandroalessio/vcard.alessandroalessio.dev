...
function ItemClipboard(props) {
	const [data, setData] = useState(null);

	const CopyData = (e) => {
		var clipboardInput = e.currentTarget.querySelector('input')
			? e.currentTarget.querySelector('input').value
			: '';
		navigator.clipboard.writeText(clipboardInput);
		document
			.getElementById('copied-clipboard-message')
			.classList.add('animation-fadeIn-fadeOut');
		setTimeout(function () {
			document
				.getElementById('copied-clipboard-message')
				.classList.remove('animation-fadeIn-fadeOut');
		}, 3000);
	};
	...
	useEffect(() => {
		if (props.html) {
			...
			setData('<b>' + props.label + ':</b> ' + props.value);
		}
	}, []);
	...
	return (
		<>
			<div
				...
				onClick={CopyData}
				data-clipboard={props.clipboard}
			>
				<input type="hidden" value={props.clipboard} />
